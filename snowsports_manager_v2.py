...
class SnowsportsManager:
    ...
    def load_data(self):
        ...
        try:
            ...
            # Extract ability level from ProductDescription_1 (FT = First Timer, BZ = Bronze, etc.)
            df['AbilityLevel'] = df['ProductDescription_1'].str.extract(r'-(\w+)$')
            ability_map = {
                'FT': 'Beginner',
                'BZ': 'Bronze',
                'SL': 'Silver',
            ...
            df['AbilityLevel'] = df['AbilityLevel'].map(ability_map).fillna('Beginner')
    ...
    def print_group_summary(self):
        ...
        for group_name, members in self.groups.items():
            ...
            for i, student in enumerate(members, 1):
                ...
                ability = student.get('AbilityLevel', 'Not specified')
                print(f"{i}. {name} (Age: {age}, Level: {ability})")
    
    def save_to_excel(self, output_file=None):
        """Save groups to an Excel file with multiple sheets."""
        if not output_file:
        ...
        with pd.ExcelWriter(output_file) as writer:
            ...
            for group_name, members in self.groups.items():
                ...
                columns = [
                    'CustomerName', 'Age', 'AbilityLevel', 'BirthDate',
                    'ParentName', 'PrimaryEmergencyContact', 'PrimaryEmergencyPhone',
                    'FoodAllergy', 'Medication', 'SpecialCondition'
                ]
            ...
            for group_name, members in self.groups.items():
                for student in members:
                    summary_data.append({
                        'Group': group_name,
                        'Name': student.get('CustomerName', ''),
                        'Age': student.get('Age', ''),
                        'Ability': student.get('AbilityLevel', ''),
                        'Emergency Contact': student.get('PrimaryEmergencyContact', ''),
                        'Phone': student.get('PrimaryEmergencyPhone', '')
                    })
